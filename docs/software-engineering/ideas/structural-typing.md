...
综上，类型兼容的条件可以概括为发送端的操作空间大于接收端的操作空间：$S_o\geq S_i$。
...
明白了这个后，再看当 a 赋给 b 后，是谁在给 a 和 b 传参：
...
综上，函数参数类型兼容的条件是：$S_i\geq S_o$，这种现象被称为逆变。
...
即函数返回值类型兼容的条件是：$S_o\geq S_i$，这种现象被称为协变。
