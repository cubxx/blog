...
type Equal<X, Y> = (<T>() => T extends X ? 1 : 2) extends <T>() => T extends Y ? 1 : 2
    ...
    : false;
/**取元组最大值 */
type Maximum<T extends any[], U = T[number], N extends any[] = []> = T extends []
    ? never
...
/**联合类型 */
namespace union {
    type Last<T> = convert.UnionToIntersection<T extends any ? (e: T) => void : never> extends (
        e: infer U,
    ) => void
        ? U
        : never;
...
namespace convert {
    ...
    type UnionToIntersection<T> = (T extends any ? (e: T) => void : never) extends (
        ...
        : never;
    /**联合类型转元组 */
    type UnionToTuple<T, S extends any[] = [], U = union.Last<T>> = U extends never
        ? S
        : UnionToTuple<Exclude<T, U>, [U, ...S]>;

    /**交叉类型转联合类型 */
    type IntersectionToUnion<T> = (T extends any ? (e: T) => void : never) extends (
        e: infer U,
    ) => void
        ? U
        : never;
