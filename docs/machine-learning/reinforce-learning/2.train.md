...
使用 [RM](../train#robbins-monro-rm) 算法逼近 $v_\pi$：
...
和 [TD](#td-估计状态价值) 一样，使用 [RM](../train#robbins-monro-rm) 算法逼近 $q_\pi$。要想进行一次价值估计的更新，则需要 $(s_t,a_t,r_{t+1},s_{t+1},a_{t+1})$：
...
要找到 $J(w)$ 最小值对应的 $w$，通常使用 [梯度下降](../train#gradient-descent-gd-梯度下降)：
$$w_{k+1}=w_k-\alpha_k\nabla_w J(w_k)$$
...
2. 对目标网络进行迭代训练，每次迭代进行 [MBGD](../train#gradient-descent-gd-梯度下降)：
